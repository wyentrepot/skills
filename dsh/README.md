...
> 注：`brainstorming`、`writing-plans`、`test-driven-development` 已迁至
> `shared/`（Kilo + Reasonix + DSH 三环境共用），不再属于本目录独有，见
...
- `brainstorming`（现位于 `shared/development-process/brainstorming`）的
  视觉伴侣包含 Windows 说明：Node 服务
